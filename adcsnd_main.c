...
static osMutexId_t m_mutex, t_mutex;
static osMessageQueueId_t measq, sendq_id;

static volatile uint32_t sigEnergy;
    ...
    for (;;)
    {
        ...
        if(false == m_sending)
        {
            ...
            if ((NULL != packet)&&(max_len > sizeof(mist_frag_message_t)))
            {
                ...
                packet->seq = (uint32_t)sigEnergy;
                packet->frag = 0;
                packet->frags = 1;

				while(osMutexAcquire(t_mutex, 1000) != osOK);
                len += encode_temperature_payload(packet->data, max_len - len,
                                                  current_temp, seqnum,
                                                  node_eui, (uint8_t*)resource_feature);
				osMutexRelease(t_mutex);
...
/* Configure ADC for stream measurement and periodically perform measurements. */
void adc_loop ()
{
	float measEnergy;
	...
	//init LDMA for ADC->memory data transfer
	ldmainit();

	//ADC init, also creates LDMA descriptor for ADC->memory transfer
	adcInit();

	for (;;)
    {

		adcStartSampling(measq);
		debug("adc started");

		//wait for measurements
		if(osOK == osMessageQueueGet(measq, &measEnergy, NULL, osWaitForever))
		{
			debug("signal energy %lu", (uint32_t) measEnergy);
			if(osMessageQueuePut(sendq_id, (uint32_t*)&measEnergy, 0U, 0U) != osOK)
			{
				//HANDLE ERROR
			}
		}

		//wait until next measurements (ADC is stopped automatically after measurement)
		osDelay(2000);
	}
}
...
int main ()
{
	...
	// Create a two element queue to receive ADC readings from ADC interrupt handler
	const osMessageQueueAttr_t measq_attr = {.name = "measq"};
	measq = osMessageQueueNew(2, sizeof(float), &measq_attr);

	// Create a four element queue to send signal energy measurements
	const osMessageQueueAttr_t sendq_attr = {.name = "sendq"};
	sendq_id = osMessageQueueNew(4, sizeof(uint32_t), &sendq_attr);
    ...
    const osThreadAttr_t adc_thread_attr = { .name = "adc" };
    osThreadNew(adc_loop, NULL, &adc_thread_attr);
